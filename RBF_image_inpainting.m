...
function [im, im_rec]=RBF_image_inpainting(image_name,spacing,sigma,lambda)
 ...
 for j=CENTER_SPACING+1:PATCH_SIZE:size(im,1)-(PATCH_SIZE+CENTER_SPACING)
  ...
  if (length(idx_fill)>0)
    ...
    if (length(idx_data)<=length(C(:)))
     ...
     [w_R]=TrainRBFRegression_regularized(z_R,PP,C,SIGMA,lambda);
    
     % Green channel
     patch_G=patch(:,:,2);
     z_G=patch_G(idx_data);
     % Solve for the weights of the RBF model
     [w_G]=TrainRBFRegression_regularized(z_G,PP,C,SIGMA,lambda);

     % Blue channel
     patch_B=patch(:,:,3);
     z_B=patch_B(idx_data);
     % Solve for the weights of the RBF model
     [w_B]=TrainRBFRegression_regularized(z_B,PP,C,SIGMA,lambda);
